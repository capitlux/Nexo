...
class LeroyCatalog extends ICatalog {
    constructor(id) {
        super(id);

        var that = this;
        that.listeningRegExp = /leroy\.inoshop\.net\/index\.php/m;
        that.loginRegExp = /leroy\.inoshop\.net\/connect_v2\/index\.php/m;
        ...
        if (that.listeningRegExp.test(window.location.href)) {
            ...
            var actualCode = '(' + function () {
                ...
                $(document).ajaxSuccess(function (event, request, settings, data) {
                    ...
                    if (settings.url.startsWith('include_client/wpp_lad/ajax/recherche.php')) {

                        //return data to content script
                        if (!data.includes('include_client/wpp_lad/ajax/recherche_exact_choix_article.php'))
                            document.dispatchEvent(new CustomEvent('HasResult', { detail: { isWaitingCrossResult: data.includes('$("#EquivalenceAutomatique").show().load("include_client/wpp_lad/ajax/recherche.php'), data: data } }));

                    } else if (settings.url.startsWith('include_client/wpp_lad/ajax/recherche_exact_choix_article.php')) {
            ...
            document.addEventListener('HasResult', function (e) {
                that.getStorageDataAsync(function (storage) {
                    ...
                    $('<div>' + e.detail.data + '</div>').find('.piece').each(function () {
                        ...
                        results.push({
                            vendor: that.id,
                            make: $(this).find('td:eq(7)').text().cleanText(),
                            ...
                            quantity: $(this).find('td:eq(26)').text().toNumber(),
                            available: $(this).find('td:eq(26)').text().toNumber() > 0,
