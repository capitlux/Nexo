...
function buildColumns() {
    ...
    lastSearchResponse.configCatalogList.forEach(function (config) {
        result.push({
            ...
            width: 0,
            template: '#=formatCell(data.' + config.id + ')#'
        });
    });
        ...
        filterRules.push({
            operator: function (item, value) {
                for (var i = 0; i < lastSearchResponse.configCatalogList.length; i++) {
                    var catalogConfig = lastSearchResponse.configCatalogList[i];
                    if (item[catalogConfig.id].available == true)
                        return true;
                }
                return false;
            }
...
function filterDataGrid() {
    ...
    for (var i = 0; i < lastSearchResponse.configCatalogList.length; i++) {
        ...
        if (grid.dataSource.data().filter(function (item) {
...
function formatCell(data) {
    ...
    var showOnlyAvailable = $('.filterAvailable input:radio:checked').val() == 'available';
    if (showOnlyAvailable && !data.available)
        return $root.html();
