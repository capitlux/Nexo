...
class DasirCatalog extends ICatalog {
    constructor(id) {
        super(id);

        var that = this;
        that.listeningRegExp = /dasir\.inoshop\.net\/index\.php/m;
        that.loginRegExp = /dasir\.inoshop\.net\/connect_v2\/index\.php/m;
        ...
        if (that.listeningRegExp.test(window.location.href)) {
            ...
            var actualCode = '(' + function () {
                ...
                $(document).ajaxSuccess(function (event, request, settings, data) {
                    ...
                    if (settings.url.startsWith('include_client/dasir/ajax/recherche.php')) {

                        //return data to content script
                        if (!data.includes('include_client/dasir/ajax/recherche_exact_choix_article.php'))
                            document.dispatchEvent(new CustomEvent('HasResult', { detail: { isWaitingCrossResult: data.includes('$("#EquivalenceAutomatique").show().load("include_client/dasir/ajax/recherche.php'), data: data } }));

                    } else if (settings.url.startsWith('include_client/dasir/ajax/recherche_exact_choix_article.php')) {
